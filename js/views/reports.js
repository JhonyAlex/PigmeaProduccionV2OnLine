        ...
        try {
            ...
            setTimeout(() => {
                ...
                if (!mainContent || mainContent.innerHTML.trim() === '') {
                    console.error("ReportsView.init: .main-content is empty. Aborting autoGenerateReport. Please check ReportsView.render() to ensure it populates the main content area.");
                    // Optionally, display a user-friendly error message in the UI
                    if (mainContent) { // If mainContent div exists but is empty
                        mainContent.innerHTML = `<div class="alert alert-danger m-3">Error: No se pudo cargar el contenido principal de la vista de reportes. Es posible que la función de renderizado de la vista no haya funcionado correctamente.</div>`;
                    } else { // If .main-content div itself is missing
                        UIUtils.showAlert('Error crítico: El contenedor principal para la vista de reportes no fue encontrado en el DOM.', 'danger');
                    }
